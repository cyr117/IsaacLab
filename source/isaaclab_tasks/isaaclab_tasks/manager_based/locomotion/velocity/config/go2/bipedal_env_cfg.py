...
class BipedalGaitReward(ManagerTermBase):
    ...
    vertical hop is body-x velocity (unpenalized once standing) and pronking earns
    the air-time term just like stepping. This term rewards the rear feet being OUT
    of phase -- one in stance while the other swings -- which produced human-like
    alternation in our pre-reproduction runs.
    """
...
def rear_feet_double_air(env: ManagerBasedRLEnv, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    term this eliminated pronking in our pre-reproduction runs.
    """
